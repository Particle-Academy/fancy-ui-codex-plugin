...
2. **Install.** Call the registry's *install-instructions* tool for the chosen component. It returns BOTH paths:
   ...
   - `npx fancy-cli@latest add <slug>` — vendor the source into the repo, shadcn-style.
...
with `codex plugin add fancy-ui@fancy-ui` after adding the Fancy UI marketplace,
and approve the
